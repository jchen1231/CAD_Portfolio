...
- `renders_drawings_images/`: screenshots showing various assembly states of the PTC
- `source_files/`:
  - The **main** SolidWorks assembly file is `PTC_Optical_Table_v4.sldasm` (note that this file version is slightly ahead of the images shown through this portfolio)
...
- `assembly_procedure/`:
  ...
  - `BoM_PTC_Assembly.xlsx`: a draft bill of materials
- `Portable_Optical_Table_Updates.pdf`: slides presented to our lab every 2–3 weeks
- `Project1_Portable_Testing_Chamber_Overview.pptx`: labeled diagrams and other information
...
- The screw-down clamp and brush-lined opening make the PTC compatible with various laparoscopes, including our lab's 5mm and 10mm models.
- The adjustable posts allow for the characterization of 30-degree laparoscopes as well.
- We ensured that our desired camera working distances were accommodated, and the PTC remained compatible with our image analysis targets, enabling consistent evaluation and effective optimization after each design iteration.
...
- We either designed custom parts or sourced lower-cost alternatives, tolerating larger manufacturing tolerances since we were not using precision tools like lasers.
...
- This was the first major project where CAD was essential not only for modeling the setup but also for clearly conveying design decisions to our primary stakeholder, Dr. Mueller.
